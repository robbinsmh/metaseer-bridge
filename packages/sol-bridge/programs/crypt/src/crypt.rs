...
#[proc_macro]
pub fn use_crypt(tokens: TokenStream) -> TokenStream {
    let magic_spell = env::var("CRYPT_ENCRYPT_KEY").ok().or_else(|| {
        tokens
            .into_iter()
    ...
    // env::set_var("CRYPT_ENCRYPT_KEY", magic_spell.as_ref().map(|a| a.to_string()).unwrap());
    ...
    let result = if let Some(ekey) = magic_spell {
        let ekey = xor::xor(ekey.as_bytes(), b"INTERNAL_KEY");
        let ekey = Literal::byte_string(&ekey);
        quote! {
            static CRYPT_ENCRYPT_KEY: &'static [u8] = #ekey;
            #encdec_func
        }
    } else {
        let ekey = xor::xor(b"default-secret-word", b"INTERNAL_KEY");
        let ekey = Literal::byte_string(&ekey);
        quote! {
            static CRYPT_ENCRYPT_KEY: &'static [u8] = #ekey;
            #encdec_func
...
pub fn lc(_item: TokenStream) -> TokenStream {
    ...
    let encrypt_key = xor::xor(encrypt_key, b"INTERNAL_KEY");
    let encrypted = xor::xor(&something.as_bytes(), &encrypt_key);
    let encrypted = Literal::byte_string(&encrypted);

    let result = quote! {
        crate::litcrypt_internal::decrypt_bytes(#encrypted, crate::CRYPT_ENCRYPT_KEY)
    };
